...
while (($startday,$endday,$type,$loc,$leader,$band,$comments,$loc_type) = $sth->fetchrow_array()) {
	($norm_style, $norm_verbstyle) = get_style($loc);
	$stylehash{$loc} = $norm_verbstyle . " -- " . $norm_style if ! $stylehash{$loc};
}
#
...
foreach $i (sort keys %stylehash) {
	print "    <option value=\"$i\">" . $stylehash{$i} . "</option>\n";
}
print "</select>\n";
...
sub get_style {
	...
	$qry = "SELECT key, type FROM style";
	$qry .= " WHERE sey = '" . $style . "'";
	$sth = $dbh->prepare($qry);
	$sth->execute();
