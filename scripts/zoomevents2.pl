...
##
## current usage:
## public_html/scripts/make-frontpage-files.sh:perl scripts/zoomevents2.pl > zoomevents2.html
##
use strict;
use Time::Local;
use DBI;
...
if ($numdays ne ""  && $numdays > 1) {
	...
	$last_mon++;
	$last_mon = sprintf "%02.d", $last_mon;
	$last_day = sprintf "%02.d", $last_day;
	$last_year += 1900;
...
my ($query_year);
$query_year = "";
if (defined $s_year && ($s_year ne $today_year)) {$query_year = $s_year};
...
$qrystr .= " AND ( type LIKE '%ZOOM%'  OR type LIKE '%ONLINE%'  OR type LIKE '%TIME%') )";
...
while ($sth->fetch) {
	...
	if (defined $comments) { $comments =~ s/<q>/"/g} ;
	if ($endday ne "") {
...
print "\n<!-- band=\"" . $band . "\" -->";
print "\n<!-- loc=\"" . $loc . "\" -->";
       #23Aug2020 APW Don't look up the venue if it's blank
	...
	# 8May2020 edb rearrange order of field data to be more palatable
		...
		print "<em>$comments</em>.\n";
                ...
                print "<strong>$type</strong>";
        # 23Aug2020 apw don't print a location if it's blank in the DB
                if ($loc ne "") {
...
print "\n<!-- band=\"" . $band . "\" -->\n";
		    ...
		    || ($loc =~ /FACEBOOK/i )) && ($type =~ /workshop/i))
		{
		    ...
		    print "<!-- bingo -->\n";
		    print " with music by ", $band if ($band !~ /^$/) & $leader ne "";
		
       # 23Aug2020 apw if $location not blank, do "Music by", otherwise just blat out the band.
                    print " Music by ", $band if ($band !~ /^$/ && $leader eq "" && $loc ne "");
		    print $band if ($band !~ /^$/ && $leader eq "" && $loc eq "");
               }
	# 8May2020 edb rearrange order of field data to be more palatable
	        if (defined($comments) && $comments !~ /^$/) { print "<em>$comments</em>" . ".\n";}
	}
